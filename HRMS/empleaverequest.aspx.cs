    ...
    public partial class empleaverequest : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            ...
            var bind = (from ep in dx.tbl_employeeleave
                        ...
                        join us in dx.tbl_employee_conectivity on e.id equals us.employe_fk
                        join es in dx.tbl_leave_type on ep.fk_leavetypeid equals es.id
                        where us.userid_fk == uid
                        select new
                        {
                            ep.id,
                            e.EmployeeCode,
        ...
        public void addleave(DateTime set)
        {
            ...
            if (chk == null && chk2 == null)
            {
                ...
                int uid = int.Parse(Session["userid"].ToString());
                var emp_query= (from ep in dx.tbl_login
                             join e in dx.tbl_employee_conectivity on ep.id equals e.userid_fk
                                where ep.id ==uid select e).FirstOrDefault();
                int empid = int.Parse(emp_query.employe_fk.ToString());
                dep.fk_empid = empid;
                ...
                dx.SaveChanges();
                var emp = (from a in dx.tbl_Employe where empid == a.id select a).FirstOrDefault();
                string content = "Employee:" + emp.EmployeeCode + " added a leave for '" + set + "'";
                var admin = (from a in dx.tbl_login where a.User_type == 1 select a).ToList();
                for (int i = 0; i < admin.Count; i++)
                    custome_class.generate_noti(admin[i].id, content);

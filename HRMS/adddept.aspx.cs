...
namespace HRMS
{
    public partial class adddept : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            var bind = (from a in dx.tbl_dept where a.Status == true select a).ToList();
        ...
        {
            tbl_dept dep = new tbl_dept();
            dep.Department_name = txtdeptname.Text;
            dep.Status = true;
            dx.tbl_dept.Add(dep);
            dx.SaveChanges();
            bind();
        ...
        protected void rptdept_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case ("Delete"):
                    ...
                    string meassage = custome_class.deletedept(id);
                    if (meassage == "success")
                    {
        ...
        {
            var bind = (from ep in dx.tbl_dept
                        where ep.id == id
                        select new
                        {
                            ep.id,
            ...
            if (bind != null)
            {
                ...
                Save.Visible = false;
                txtdeptname.Text = bind.Department_name.ToString();
        ...
        private void updatedept()
        {
            ...
            var chk = (from a in dx.tbl_dept where a.id == dept_id select a).FirstOrDefault();


                if (chk != null)
                {
                    chk.Department_name=txtdeptname.Text;
                    dx.SaveChanges();
                    bind();

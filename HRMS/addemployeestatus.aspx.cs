...
namespace HRMS
{
    public partial class addemployeestatus : System.Web.UI.Page
    {
        ...
        public void bind()
        {
            var bind = (from a in dx.tbl_Employee_status where a.status == true select a).ToList();
        ...
        {
            tbl_Employee_status dep = new tbl_Employee_status();
            dep.Employee_status_name = txtempstatus.Text;
            dep.status = true;
            dx.tbl_Employee_status.Add(dep);
            dx.SaveChanges();
            bind();
        ...
        protected void rptdept_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case ("Delete"):
                    ...
                    string meassage = custome_class.deleteempstatus(id);
                    if (meassage == "success")
                    {
        ...
        private void bindupdate(int id)
        {
            var bind = (from ep in dx.tbl_Employee_status
                        where ep.id == id
                        select new
                        {
                            ep.id,
                            ep.Employee_status_name,
                            ep.status,
            ...
            if (bind != null)
            {
                ...
                Save.Visible = false;
                txtempstatus.Text = bind.Employee_status_name.ToString();
        ...
        {
            int s_id = int.Parse(d_id.Value.ToString());

            var chk = (from a in dx.tbl_Employee_status where a.id == s_id select a).FirstOrDefault();


            if (chk != null)
            {
                chk.Employee_status_name = txtempstatus.Text;
                dx.SaveChanges();
                bind();

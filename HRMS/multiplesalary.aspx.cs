...
namespace HRMS
{
    public partial class multiplesalary : System.Web.UI.Page
    {
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (RepeaterItem item in rptsalaryslip.Items)
                {
                    ...
                    HiddenField lblempid = (HiddenField)item.FindControl("HiddenID");
                    long empid = long.Parse(lblempid.Value);

                    var chk = (from a in dx.tbl_salary where a.fk_emp_id ==empid && a.salary_month == month && a.salary_year == year select a).FirstOrDefault();
                    if (chk != null)
                    {

                        Control divsalaryslips = (Control)item.FindControl("divsalaryslips");
                  
                        string fl = custome_class.Encrypt(lblempcode.Value + ddlmonth.SelectedItem.Text +
                            ddlyear.SelectedItem.Text, "");
                        string filename = fl + ".pdf";
                        salary_class.print_salary_slip(divsalaryslips, filename);

                        chk.slip_url = "PDF/" + filename;
                        dx.SaveChanges();
                ...
                //Response.RedirectLocation = "multiplesalary.aspx";
                ClientScript.RegisterStartupScript(GetType(), "Hide", "<script> $('#custom-width-modal').modal('hide');</script>");

            }
            catch (Exception ex) {
                this.ClientScript.RegisterStartupScript(this.GetType(), "SweetAlert", "swal('ERROR!', "+ex.Message+", 'error');", true);
            }
        ...
        public void save_salary() {
            foreach (RepeaterItem item in rptleavetype.Items)
            {
                ...
                var chk = (from a in dx.tbl_salary where a.fk_emp_id == epid && a.salary_month == month && a.salary_year == year select a).FirstOrDefault();
                if (chk == null)
                {
        ...
        public void bind_slip()
        {
            ...
            if (ddldept.SelectedItem.Text != "All") {
                long dept = Convert.ToInt32(ddldept.SelectedValue);

                binds = from ep in result
                        join e in dx.tbl_Employe on ep.empid equals e.id
                        join d in dx.tbl_employee_conectivity on e.id equals d.employe_fk
                        join c in dx.tbl_dept on d.Dept_fk equals c.id
                        join j in dx.tbl_job_descrion on d.jobtitle_fk equals j.id
            ...
            var all = from ep in binds.AsEnumerable()
                      let absents = salary_class.get_absent_deduction(ep.EmployeeID, year, month, Convert.ToInt64(ep.amount))
                      let lates = salary_class.get_late_deduction(ep.EmployeeID, year, month, Convert.ToInt64(ep.amount))
                      ...
                      select new
                      {
                          ...
                          late_days= late_days,
                              ...
                              in_words= custome_class.NumberToWords(Convert.ToInt64(ep.amount - total_decuction)),
                              user_name=username
                    ...
                    if (loan == false)
                    {
                        var chng = from ep in binds.AsEnumerable()
                                   ...
                                   select new
                                   {
                                       ...
                                       salary_month = ddlmonth.SelectedItem.Text,
                                       salary_year = ddlyear.SelectedItem.Text,
                                       in_words = custome_class.NumberToWords(Convert.ToInt32(ep.amount - total_decuction)),
                                       user_name = username
